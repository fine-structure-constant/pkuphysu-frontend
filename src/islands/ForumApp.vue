...
interface ForumPost { id: number; author: string; authorId: number; createdAt: number; title: string; excerpt: string; content: string; contentHtml: string; tags: string[]; replies: number; likes: number; follows: number; liked: boolean; followed: boolean; commentPreview: ForumComment[]; previewLoading: boolean; }
...
const loadCommentPreview = async (post: ForumPost) => {
  if (!post.replies) return;
  post.previewLoading = true;
  try { post.commentPreview = ((await apiJson<ApiComment[]>(`/forum/comments/${post.id}?limit=3&begin=0&sort=desc`)).data || []).map(commentFromApi); }
  catch { post.commentPreview = []; }
  finally { post.previewLoading = false; }
};
const loadCommentPreviews = async (loadedPosts: ForumPost[]) => {
  const queue = loadedPosts.filter((post) => post.replies > 0);
  const worker = async () => { let post: ForumPost | undefined; while ((post = queue.shift())) await loadCommentPreview(post); };
...
const loadPosts = async () => {
  ...
  try {
    ...
    void loadCommentPreviews(posts.value);
...
onMounted(async () => { if (!getAuthSession()?.token) { window.location.replace(`/login?next=${encodeURIComponent(window.location.pathname)}`); return; } window.addEventListener("keydown", onKeydown); try { await apiJson("/user/me"); document.body.classList.add("public-auth-ready"); await Promise.all([loadTags(), loadPosts()]); } catch (error) { handleError(error); } queryInput.value?.focus(); });
onBeforeUnmount(() => { window.removeEventListener("keydown", onKeydown); window.clearTimeout(toastTimer); document.body.classList.remove("public-auth-ready"); });
</script>

<template>
  <div class="forum-app page-wrap"><div class="forum-layout" :class="{ 'has-detail': selectedPost }">
    ...
    <section class="forum-stream" aria-labelledby="forum-stream-title"><div class="forum-toolbar glass-panel"><div><p class="eyebrow">THREADS / {{ browseType === 'posts' ? 'LATEST' : browseType === 'follow' ? 'FOLLOWING' : 'COMMENTS' }}</p><h2 id="forum-stream-title">{{ browseType === 'follow' ? '我关注的讨论' : browseType === 'comments' ? '正在发生的讨论' : '最新讨论' }}</h2></div><label class="forum-search"><Icon name="search" :size="19" /><input ref="queryInput" v-model="query" type="search" placeholder="搜索讨论，或使用 #ID / :标签" aria-label="搜索论坛" @change="loadPosts" /><kbd>⌘ K</kbd></label></div><div v-if="query" class="forum-query-hint">正在搜索 <strong>{{ query }}</strong><span>关键词 · #ID · :标签均可</span></div><div class="forum-post-list"><div v-if="loading" class="forum-empty glass-panel"><span class="empty-state__mark">…</span><div><strong>正在读取讨论</strong><p>请稍候。</p></div></div><article v-for="post in filteredPosts" v-else :key="post.id" class="forum-post glass-panel"><div class="forum-post__head"><div class="avatar avatar--blue">{{ getInitials(post.author).slice(0, 1) }}</div><div class="forum-post__author"><strong>{{ post.author }}</strong><span>#{{ post.authorId }} · {{ displayTime(post.createdAt) }}</span></div><button class="icon-button forum-post__more" type="button" aria-label="更多操作"><Icon name="more" :size="17" /></button></div><div class="forum-post__content" role="button" tabindex="0" @click="openPost(post)" @keydown.enter="openPost(post)" @keydown.space.prevent="openPost(post)"><h3>{{ post.title }}</h3><MarkdownContent :html="post.contentHtml" class="forum-markdown--excerpt" /></div><div class="forum-post__foot"><div class="tag-list"><span v-for="tag in post.tags" :key="tag" class="tag-chip">{{ tag }}</span></div><div class="post-actions" aria-label="帖子互动"><button :class="{ active: post.followed }" type="button" :aria-label="`${post.followed ? '取消关注' : '关注'}，${post.follows} 人关注`" @click="toggleFollow(post)"><span class="post-action__icon"><Icon name="star" :size="15" /></span><span class="post-action__count">{{ post.follows }}</span></button><button :class="{ active: post.liked }" type="button" :aria-label="`${post.liked ? '取消点赞' : '点赞'}，${post.likes} 个赞`" @click="toggleLike(post)"><span class="post-action__icon"><Icon name="heart" :size="15" /></span><span class="post-action__count">{{ post.likes }}</span></button><button type="button" :aria-label="`查看 ${post.replies} 条评论`" @click="openPost(post)"><span class="post-action__icon"><Icon name="message" :size="15" /></span><span class="post-action__count">{{ post.replies }}</span></button></div></div><div v-if="post.replies" class="forum-comment-preview" :aria-label="`帖子 ${post.id} 的最新评论`"><div class="forum-comment-preview__heading"><span><Icon name="message" :size="15" />最新评论</span><button type="button" @click="openPost(post)">查看全部 {{ post.replies }} 条</button></div><p v-if="post.previewLoading && !post.commentPreview.length" class="forum-comment-preview__loading">正在读取评论……</p><div v-for="comment in post.commentPreview" :key="comment.id" class="forum-comment-preview__item"><div class="avatar avatar--small avatar--ocean">{{ getInitials(comment.author).slice(0, 1) }}</div><div><p><strong>{{ comment.author }}</strong><time>{{ displayTime(comment.createdAt) }}</time></p><MarkdownContent :html="comment.contentHtml" class="forum-markdown--comment-preview" /></div></div></div></article><div v-if="!loading && !filteredPosts.length" class="forum-empty glass-panel"><span class="empty-state__mark">○</span><div><strong>{{ emptyText }}</strong><p>试试切换视图，或清除当前筛选。</p><button class="text-button" type="button" @click="query = ''; browseType = 'posts'; void loadPosts()">清除筛选</button></div></div></div><div class="forum-end"><span>—</span><p>已到达当前 API 返回列表的末尾</p><span>—</span></div></section>
